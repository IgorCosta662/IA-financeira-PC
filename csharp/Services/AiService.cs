...
namespace FinancaAI.Services
{
    public class AiService
    {
        ...
        {
            // Build the system instructions matching server.ts personalities
...
Não faça rodeios emocionais. Trate tudo como equações puras. Use formatação de código se útil e liste cálculos detalhados para embasar suas teses.";
            ...
            // Summarize financial status context if provided
            ...
            if (request.FinancialData != null)
            {
                ...
                if (request.FinancialData.Cards != null && request.FinancialData.Cards.Count > 0)
                {
                    ...
                    {
                        sb.AppendLine($"  * {card.Bank} {card.CardName} ({card.Brand}): Limite {request.FinancialData.Settings?.Currency} {card.Limit:N2} (Vence dia {card.DueDay})");
                    }
                }
                ...
                if (request.FinancialData.Investments != null && request.FinancialData.Investments.Count > 0)
                {
                    ...
                    sb.AppendLine($"  * Total Investido: {request.FinancialData.Settings?.Currency} {totalInvested:N2} | Patrimônio em investimentos: {request.FinancialData.Settings?.Currency} {totalCurrent:N2}");
                }

                if (request.FinancialData.Debtors != null && request.FinancialData.Debtors.Count > 0)
                {
                    sb.AppendLine("- Devedores (Dinheiro a receber de terceiros):");
                    foreach (var deb in request.FinancialData.Debtors)
                    {
                        sb.AppendLine($"  * {deb.Name}: {request.FinancialData.Settings?.Currency} {deb.Amount:N2} (Status: {deb.Status})");
                    }
                }

                if (request.FinancialData.Creditors != null && request.FinancialData.Creditors.Count > 0)
                {
                    sb.AppendLine("- Credores (Dívidas/Financiamentos a pagar):");
                    foreach (var cred in request.FinancialData.Creditors)
                    {
                        sb.AppendLine($"  * {cred.CreditorName}: Total {request.FinancialData.Settings?.Currency} {cred.Amount:N2} (Parcela {cred.CurrentInstallment}/{cred.InstallmentsCount})");
                    }
                }
                ...
                if (request.FinancialData.Goals != null && request.FinancialData.Goals.Count > 0)
                {
                    ...
                    {
                        decimal pct = goal.TargetAmount > 0 ? (goal.CurrentAmount / goal.TargetAmount) * 100 : 0;
                        sb.AppendLine($"  * {goal.Name}: Economizado {goal.CurrentAmount:N2} de {goal.TargetAmount:N2} ({pct:F1}% concluído) - Prazo: {goal.Deadline}");
                    }
                }
                ...
                if (request.FinancialData.Transactions != null && request.FinancialData.Transactions.Count > 0)
                {
                    ...
                    foreach (var t in request.FinancialData.Transactions)
                    {
                        ...
                        sb.AppendLine($"  * [{t.Date}] {t.Description} ({t.Category}) -> {request.FinancialData.Settings?.Currency} {t.Amount:N2} [{t.Type}]");
                    }
                }
...
1. Forneça conselhos realistas baseados nos dados fornecidos do usuário (se houver). Se não houver saldo ou transações, ensine o usuário a criar registros primeiro.
2. Seja proativo e sugira formas práticas de economizar, gerenciar dívidas ou organizar orçamentos.
3. Jamais sugira investimentos fraudulentos, apostas esportivas, cassinos ou esquemas de enriquecimento rápido.
...
5. Seja focado em soluções práticas para quitar dívidas e economizar. Utilize formatação Markdown limpa (tabelas, listas ou negritos) para tornar as respostas altamente escaneáveis.";
        ...
        private async Task<AiChatResponse> CallGeminiApiAsync(AiChatRequest request, string systemInstruction, string? serverGeminiKey)
        {
            ...
            // Map chat history and user message to user parts
            var historyText = new StringBuilder();
            if (request.History != null && request.History.Count > 0)
            {
                historyText.AppendLine("Histórico de nossa conversa:");
            ...
            try
            {
                ...
                return new AiChatResponse(text ?? "Desculpe, o modelo não retornou conteúdo textual.");
            }
            catch (Exception ex)
            {
        ...
        private async Task<AiChatResponse> CallOpenAiCompatibleApiAsync(AiChatRequest request, string systemInstruction, string provider)
        {
            ...
            if (provider == "openai")
            {
                ...
                if (string.IsNullOrWhiteSpace(apiKey))
                {
                    return new AiChatResponse("Chave de API da OpenAI não configurada. Por favor, adicione sua chave nas Configurações.");
                }
            }
            else if (provider == "nvidia")
            {
                ...
                if (string.IsNullOrWhiteSpace(apiKey))
                {
                    return new AiChatResponse("Chave de API da NVIDIA não configurada. Por favor, adicione sua chave nas Configurações.");
                }
            }
            ...
            try
            {
                ...
                {
                    string errorMsg = await response.Content.ReadAsStringAsync();
                    return new AiChatResponse($"Erro na API do Provedor ({provider}): {response.StatusCode} - {errorMsg}");
                }
            ...
            {
                return new AiChatResponse($"Exceção ao chamar a API do Provedor ({provider}): {ex.Message}");
            }
        }
    }
}
